...
pub enum Error {
    ...
    #[error("the {} field name: '{}' is invalid: {:?}", .0.field_type, .0.field_name, .0.path)]
    ...
    DataExist(String),
    #[error("space of page is not enough")]
    PageSpaceNotEnough,
    #[error("data has no primary key")]
    DataHasNoPrimaryKey,
    #[error("journal's checksum is mismatched with datas, database maybe corrupt")]
    ChecksumMismatch,
    #[error("journal's page size is mismatched with database. expect: {0}, actual: {1}")]
